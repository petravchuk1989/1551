...
begin 
	SELECT [2000],[2001],[2002],[2003],[2004],[2005],[2006],[2007],[2008],[2009]
			 FROM ( select * from (
								  SELECT [RDAId],
								   IndexOfFactToExecution as [Value]
	 ...
	 SELECT  [2000],[2001],[2002],[2003],[2004],[2005],[2006],[2007],[2008],[2009]
			 FROM ( select * from (
								  SELECT [RDAId],
								   cast([IndicatorPlace] as numeric(18,2)) as [Value]
							  FROM [CRM_1551_Rating].[dbo].[Rating_IndexToFactExecution]
							  where [RDAId] is not null
							  and [RatingId] = @RatingId
							  and [DateCalc] = @DateCalc
	) as Table123) x
...
/*
	...
	from (
							SELECT AVG(cast([IndicatorPlace] as numeric(18,2))) as [Value]
							  FROM [CRM_1551_Rating].[dbo].[Rating_IndexToFactExecution]
							  where [RDAId] is not null
							  and [RatingId] = @RatingId
							  and [DateCalc] = @DateCalc
							  and [RDAId] = @RDAId
							  and [IndicatorPlace] is not null
							group by [RDAId]
	) as Table123
*/
