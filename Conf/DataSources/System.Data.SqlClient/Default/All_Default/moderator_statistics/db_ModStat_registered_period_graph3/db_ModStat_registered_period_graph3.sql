...
declare @date_to datetime='2019-07-02 12:10:02'
...
*/
/*
select [AppealsFromSite].Id, l.EditByUserId, l.leg_name, convert(date, [ReceiptDate]) [date]
from [CRM_1551_Site_Integration].[dbo].[AppealsFromSite]
inner join #leg l on [AppealsFromSite].[EditByUserId]=l.EditByUserId
where [AppealsFromSite].AppealFromSiteResultId=3
and convert(date, [AppealsFromSite].[ReceiptDate]) between convert(date, @date_from) and convert(date, @date_to)
*/


declare @query nvarchar(max)=N'
select user_id Id, leg_name, '+@dates+N'
from
(select [AppealsFromSite].Id, l.EditByUserId user_id, l.leg_name, convert(date, [ReceiptDate]) [date]
from [CRM_1551_Site_Integration].[dbo].[AppealsFromSite]
inner join #leg l on [AppealsFromSite].[EditByUserId]=l.EditByUserId
where [AppealsFromSite].AppealFromSiteResultId=3
and convert(date, [AppealsFromSite].[ReceiptDate]) between '''+ltrim(convert(date, @date_from))+N''' and '''+ltrim(convert(date, @date_to))+N''') t
pivot
(
count(Id)
for [date] in ('+@dates+N')
