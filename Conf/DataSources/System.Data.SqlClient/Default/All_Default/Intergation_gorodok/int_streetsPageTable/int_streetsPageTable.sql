SELECT 
	...
	from [CRM_1551_GORODOK_Integrartion].[dbo].[Done_values_in_directories] as done
		...
		left join (select * from [CRM_1551_GORODOK_Integrartion].[dbo].[Programs_1551_catalog] where code ='streets' ) as cat on cat.integration_tables_id = old.id or cat.integration_tables_id = new.id
		left join dbo.Streets as st on st.Id = cat.[1551_id]
where done.is_done = 0
and done.integration_tables_code = 'street' and done.program = 'gorodok'
-- #filter_columns#
--   #sort_columns#
