...
BEGIN
	...
	begin
		update [CRM_1551_GORODOK_Integrartion].[dbo].[Gorodok_streets_old]
		set  [name] =  (select [name] from [CRM_1551_GORODOK_Integrartion].[dbo].[Gorodok_streets_new] where id = @values_id ) 
		where id = @values_id
	...
	begin
		SET IDENTITY_INSERT [CRM_1551_GORODOK_Integrartion].[dbo].[Gorodok_streets_old] ON;

		insert into [CRM_1551_GORODOK_Integrartion].[dbo].[Gorodok_streets_old] (id, [name])
		select id, [name] from [CRM_1551_GORODOK_Integrartion].[dbo].[Gorodok_streets_new] where id = @values_id

		SET IDENTITY_INSERT [CRM_1551_GORODOK_Integrartion].[dbo].[Gorodok_streets_old] OFF;

		if not exists (select * from [CRM_1551_GORODOK_Integrartion].[dbo].[Programs_1551_catalog] where code = 'streets' and integration_tables_id = @values_id and [1551_id] = @id_1551_new)
		begin
			insert into [CRM_1551_GORODOK_Integrartion].[dbo].[Programs_1551_catalog] (program, code, integration_tables_id, [1551_id])
			values ('gorodok', 'streets', @values_id, @id_1551_new)
		end
	...
	begin
		...
		delete from [CRM_1551_GORODOK_Integrartion].[dbo].[Gorodok_streets_old] where id = @delete_id
	end
END
	else
BEGIN
