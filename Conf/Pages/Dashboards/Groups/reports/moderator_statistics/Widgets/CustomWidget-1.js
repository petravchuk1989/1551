(function() {
    return {
        title: 'Зареєстровано Всього > 2 годин',
        hint: '',
        formatTitle: function() {},
        customConfig:
                    ` 
                <div id='container2' class='main-con'>
                </div>
                    `
        ,
        init: function() {
        ...
        load: function({rows}) {
            const con = document.getElementById('container2')
            ...
            const grid = `<div class='widget-container'>
                        <p class='widget-value cell-info'>${rows[0].values[2]}</p>
                        <p class='widget-value'> % порівняння: ${exam}<span class='cell-info'> ${rows[0].values[3]}</span></p>
                        </div>`;
