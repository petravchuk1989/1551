...
FOR
  select Id from [dbo].[QuestionTypes] where question_type_id = 1
OPEN @CURSOR
...
CLOSE @CURSOR
  ...
  select top 10 GroupQuestionId,
   case when GroupQuestionId <> 0 then 
   (select [name] from QuestionTypes where Id = GroupQuestionId) 
    else '' end as GroupQuestionName, isnull(sum(val),0) qty
  from #temp_OUT o
