        ...
        init: function() {
            ...
            if (this.state == "create") {
                ...
                this.queryExecutor.getValues(queryForGetValueStateServer).subscribe(data => {
                    let indexLoad =  data.columns.findIndex(el => el.code.toLowerCase() === 'statecode' );
                    if (data.rows[0].values[indexLoad] == "Simple load") {
                        this.StateServerId = 1;
                    }else if (data.rows[0].values[indexLoad] == "Standart load") {
                        this.StateServerId = 2;
                    }else if (data.rows[0].values[indexLoad] == "Hard load") {
                ...
                this.queryExecutor.getValues(queryForGetValue).subscribe(data => {
                    ...
                    let indexReceiptSources =  data.columns.findIndex(el => el.code.toLowerCase() === 'receipt_source_id' );
                    let indexUserEditId =  data.columns.findIndex(el => el.code.toLowerCase() === 'user_edit_id' );
                    let indexRegNumber =  data.columns.findIndex(el => el.code.toLowerCase() === 'registration_number' );
                    let indexReceiptSouName =  data.columns.findIndex(el => el.code.toLowerCase() === 'receiptsources_name' );
                    let indexPhoneNum =  data.columns.findIndex(el => el.code.toLowerCase() === 'phone_number' );

                    this.form.setControlValue('AppealId', data.rows[0].values[indexId]);
                    this.form.setControlValue('ReceiptSources', { key: data.rows[0].values[indexReceiptSources], value: data.rows[0].values[indexReceiptSouName] });
                    this.form.setControlValue('AppealNumber', data.rows[0].values[indexRegNumber]);
                    this.form.setControlValue('Phone', data.rows[0].values[indexPhoneNum]);
                    this.form.setControlValue('DateStart', new Date());
                    this.form.setControlValue('CardPhone', this.form.getControlValue('Phone'));
                    //LoadDetail Detail_Aplicant
                    const parameters = [
                        { key: '@phone_number', value: data.rows[0].values[indexPhoneNum] }
                    ];
                    this.details.loadData('Detail_Aplicant', parameters);
                        //Detail_QuestionReestration
                    const parameters2 = [
                        { key: '@AppealId', value: data.rows[0].values[indexId] }
                    ];
                ...
                this.interval = setInterval(function() {
                    let date = new Date();
                    this.form.setControlValue('DateEnd', date);
                }.bind(this), 1000);
