    ...
    myCreateExcel: function(data){
        ...
        for( let  j = 0; j < data.rows.length; j ++ ){  
            ...
            for( i = 0; i < indexArr.length; i ++){
                var el = indexArr[i];
                if( el.name === 'registration_number'  ){
                    rowItem.registration_number = row.values[2] + ' ' + row.values[7];
                    console.log(rowItem.registration_number)
                }else if(el.name === 'zayavnyk' ){
                    rowItem.zayavnyk = row.values[4] + ' ' + row.values[10];
                    console.log( rowItem.zayavnyk )
                }else if(el.name === 'QuestionType' ){
                    rowItem.QuestionType = 'Тип питання: ' +  row.values[3] + '. Зміст: ' + row.values[11];;
