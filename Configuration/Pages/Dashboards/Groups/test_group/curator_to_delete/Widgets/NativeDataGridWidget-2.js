    ...
    myCreateExcel: function(data){
        ...
        for( let  j = 0; j < data.rows.length; j ++ ){  
            ...
            for( i = 0; i < indexArr.length; i ++){
                var el = indexArr[i];
                if( el.name === 'registration_number'  ){
                    rowItem.registration_number = row.values[1] + ' ' + row.values[3];
                }else if(el.name === 'zayavnykName' ){
                    row.values[12] === null ?  value = '' : value = row.values[12] ;
                    rowItem.zayavnykName = row.values[2] + ' ' + value;
                }else if(el.name === 'QuestionType' ){
                    rowItem.QuestionType = 'Тип питання: ' + row.values[5] + '. Зміст: ' + row.values[11];
                }else if( el.name === 'vykonavets'  ){
                    rowItem.vykonavets = row.values[8];
                }else if( el.name === 'adress'  ){
                    rowItem.adress = row.values[6];
                }
            };
            rows.push( rowItem );
        };
