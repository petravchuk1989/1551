(function () {
  return {
    ...
    myCreateExcel: function(data){
        ...
        let column_adress = { name: 'adressZ', index: 4 };
        ...
        indexArr.forEach( el => {
            if( el.name === 'registration_number'){
                ...
                columnsHeader.push(obj);
                captions.push('Адреса');
            }else if( el.name === 'adressZ'){
                var obj =  { 
                    key: 'adressZ',
                    width: 21
                };
        ...
        for( let  j = 0; j < data.rows.length; j ++ ){  
            ...
            for( i = 0; i < indexArr.length; i ++){
                var el = indexArr[i];
                if( el.name === 'registration_number'  ){
                    rowItem.registration_number = row.values[1];
                }else if(el.name === 'zayavnikName' ){
                    rowItem.zayavnikName = row.values[3];
                }else if(el.name === 'QuestionType' ){
                    rowItem.QuestionType = 'Тип питання: ' + row.values[2] + '. Зміст: ' + row.values[9];
                }else if( el.name === 'vykonavets'  ){
                    rowItem.vykonavets = row.values[5]
                }else if( el.name === 'adressZ'  ){
                    rowItem.adressZ = row.values[10] +  ' ' + row.values[8];
                }else if( el.name === 'adress'  ){
                    rowItem.adress = row.values[4];
                }
            };
            rows.push( rowItem );
        };
